...
class r0710304:
    ...
    def optimize(self, filename):
        ...
        while solver.should_continue(timeLeft): 
            ...
            # Your code here.
            population = solver.evolutionary_iteration(distanceMatrix, population)
            ...
            timeLeft = self.reporter.report(solver.mean, solver.best, solver.bestRoute)
            if timeLeft < 0:
                break
...
class Individual:
    ...
    def computeFitness(self, distanceMatrix: np.array):
        ...
        for i in range(len(self.route)):
            dist = distanceMatrix[self.route[i]][self.route[(i+1) % len(self.route)]]
            self.fitness += dist
...
class Solver:
    ...
    def __init__(self, distanceMatrix: np.array, maxTime: int):
        ...
        self.max_time = maxTime                     # Maximum time for the algorithm to iterate
        ...
        self.its_same_best = 0                      # Amt of iterations in which the best value has not changed
        self.max_its_same_best = 100                # The maximum amt of iterations with unchanged best value

        self.ile_exploration = True                 # Whether the population is split into islands
        self.ile1 = None                            # Individuals on island 1
        self.ile2 = None                            # Individuals on island 2

        self.ile_its = int(round(self.max_its/10))  # Amount of iterations before island exchange
        self.ile_swaps = int(round(self.lambdaa/20)) # Amount of individuals swapped between iles
    ...
    def update_params(self): 
        ...
        """
        self.inversion_probability = 0.8 - 0.6*(self.iteration/self.max_its)  # Inversion Mutation probability
        self.swap_probability = 0.1 + 0.3*(self.iteration/self.max_its)       # Swap Mutation probability
    ...
    def should_continue(self, time_left: int):
        ...
        if (self.previous_best == None): self.previous_best = self.best
        if (self.previous_best == self.best): self.its_same_best += 1
        else: self.its_same_best = 0
    ...
    def elimination(self, population: List[Individual], offspring: List[Individual]):
        """ Elimination """
        if (self.ile_exploration): lambdaa = int(round(self.lambdaa/2))
        else: lambdaa = self.lambdaa
    ...
    def ile_exchange(self, ile1: List[Individual], ile2: List[Individual]):
        ...
        for _ in range (self.ile_swaps):
            index1 = k_tournament(ile1, self.k) # random.randint(0, len(ile1)-1)
            i1 = ile1.index(index1)
            index2 = k_tournament(ile2, self.k) # random.randint(0, len(ile2)-1)
    ...
    def evolutionary_iteration(self, distanceMatrix: np.array, pop: List[Individual]):
        ...
        if (self.ile_exploration):
            ...
            self.ile1 = self.evolutionary_iteration_ind(distanceMatrix, True, self.ile1, 1)
            self.ile2 = self.evolutionary_iteration_ind(distanceMatrix, True, self.ile2, 2)

            self.update_params()
            self.iteration += 1

            # Exchange 
            if (self.iteration % self.ile_its == 0): self.ile1, self.ile2 = self.ile_exchange(self.ile1, self.ile2)

            pop = self.ile1 + self.ile2

        else: 
            pop = self.evolutionary_iteration_ind(distanceMatrix, False,  pop, 1)
            self.update_params
    ...
    def evolutionary_iteration_ind(self, distanceMatrix: np.array, isIsland, pop: List[Individual], islandNb: int):
        ...
        if isIsland: its = int(np.ceil(self.mu / 4))
        else: its = int(np.ceil(self.mu / 2))
        ...
        for _ in range(its):
            ...
            if random.random() < self.recombination_probability:
                new_offspring = self.recombination(distanceMatrix, p1, p2, islandNb)
            else:
                new_offspring = [p1, p2]
...
def inversion_mutation(p: Individual):
    ...
    for _ in range(p.sigma):
        ...
        if i1 > i2:
            i1, i2 = i2, i1
        p.route = list(itertools.chain(p.route[:i1], list(reversed(p.route[i1:i2])), p.route[i2:]))
